...
import {
  addCampusAdminByMobileAction,
  removeCampusUserAction,
  updateCampusUserRoleAction,
...
export default async function PlatformCampusAdminsPage({ params, searchParams }: Props) {
  ...
  const data = await adminGraphqlExecute<{
    ...
    `
    query CampusMembers($cid: bigint!) {
  ...
  if (!data.campuses_by_pk) notFound();
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-semibold text-zinc-900">学校成员 · {data.campuses_by_pk.name}</h1>
        <p className="mt-1 text-sm text-zinc-500">
          按手机号添加用户。学校侧日常成员维护也可由该校管理员在站点后台「本校成员」中操作。
        </p>
      </div>
      ...
      <div className="mb-8 max-w-xl rounded-lg border border-zinc-200 bg-white p-6">
        ...
        <p className="mt-1 text-xs text-zinc-500">输入已注册用户的手机号，将其设为本校 admin（若已在名单中则改为 admin）。</p>
        <form action={addCampusAdminByMobileAction} className="mt-4 flex flex-wrap items-end gap-3">
              ...
              data.campus_users.map((row) => (
                <tr key={row.id} className="border-t border-zinc-100">
                  ...
                  <td className="px-4 py-3">
                    <form action={updateCampusUserRoleAction} className="flex flex-wrap items-center gap-2">
                  ...
                  <td className="px-4 py-3">
                    <form action={removeCampusUserAction} className="inline">
                      <input type="hidden" name="campus_user_id" value={row.id} />
                      <input type="hidden" name="campus_id" value={campusId} />
